...
async function fetchingRandomNews() {
    try {
        const api_url = `${proxy}https://gnews.io/api/v4/top-headlines?token=${apikey}&country=in&lang=en&from=${formattedDate}&sortby=publishedAt`;
...
async function fetchNewsQuery(query) {
    try {
        const api_url = `${proxy}https://gnews.io/api/v4/search?q=${query}&token=${apikey}&country=in&lang=en`;
        const responses = await fetch(api_url);
...
function displayBlog(articles) {
    ...
    articles.forEach((article) => {
        ...
        const truncatedDescription = article.description && article.description.length > 150
